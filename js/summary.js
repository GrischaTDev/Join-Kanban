...
function initSummary(allTasks) {
    ...
    let earliestUrgentDueDate = null; // Variable zur Speicherung des frühesten Fälligkeitsdatums für dringende Aufgaben

    // Durchlaufen Sie das allTasks-Array, um die Zähler zu aktualisieren und das früheste Fälligkeitsdatum für dringende Aufgaben zu finden
    allTasks.forEach(task => {
        if (task.progressfield === 'todo_container') {
            todoCount++;
        } else if (task.progressfield === 'done_container') {
            doneCount++;
        }

        if (task.priority && task.priority.urgent) {
            urgentCount++;
            // Überprüfen, ob der Task eine bevorstehende Frist hat
            if (task.dueDate) {
        ...
        if (task.progressfield === 'inprogress_container') {
            inProgressCount++;
        } else if (task.progressfield === 'await_feedback_container') {
            awaitFeedbackCount++;
        }
    });

    // Formatieren Sie das früheste Fälligkeitsdatum für dringende Aufgaben
    let upcomingDeadline = '';
    if (earliestUrgentDueDate) {
        const options = { month: 'long', day: 'numeric', year: 'numeric' };
        upcomingDeadline = earliestUrgentDueDate.toLocaleDateString('en-US', options);
    }
            ...
            <div class="left-column">
                <div class="summary-boxes">
                    ...
                    <div class="summary-box-row-3">
                        ...
                        <a href="./board.html" class="anchor-style box-row-3">
                            <span class="number-and-name">${inProgressCount}</span>
                            <h5>Tasks in<br> Progress</h5>
                        </a>
                        <a href="./board.html" class="anchor-style box-row-3">
                            <span class="number-and-name">${awaitFeedbackCount}</span>
